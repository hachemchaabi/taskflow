  ...
  return (
    <>
      <div style={style} {...listeners} {...attributes} ref={setNodeRef}>
        <Card
          className={cn(
            'cursor-pointer gap-4 rounded-md p-3 shadow-sm transition-opacity',
...
>({
  ...
  const clearActiveTimer = useRef<ReturnType<typeof setTimeout>>()

  useEffect(() => () => clearTimeout(clearActiveTimer.current), [])

  const releaseActiveCard = () => {
    clearTimeout(clearActiveTimer.current)
    clearActiveTimer.current = setTimeout(() => setActiveCardId(null), DROP_ANIMATION_MS)
  }
  ...
  const handleDragCancel = () => releaseActiveCard()
