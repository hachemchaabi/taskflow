...
function ToastIcon({ type }: { type?: string }): React.ReactElement | null {
  if (type === 'loading') {
    return (
      <div className="flex h-lh items-center" data-slot="toast-icon">
        <Spinner className="size-4 opacity-80" />
      </div>
    )
  }

  const iconName = type ? TOAST_ICONS[type as keyof typeof TOAST_ICONS] : null
  ...
  return (
    ...
    >
      <Icon
        name={iconName}
        className="in-data-[type=error]:text-destructive in-data-[type=info]:text-info in-data-[type=success]:text-success in-data-[type=warning]:text-warning"
      />
    </div>
  )
}
...
function Toasts({
  ...
  return (
    <Toast.Portal data-slot="toast-portal" {...portalProps}>
      ...
      >
        {toasts.map((toast) => {
          const onClick = (toast.data as { onClick?: () => void } | undefined)?.onClick
          return (
            <Toast.Root
              key={toast.id}
              onClick={
